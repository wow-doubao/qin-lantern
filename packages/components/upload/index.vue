<template>
  <div class="upload">
    ...
    <template v-else>
      ...
      <ql-dialog v-model="dialogVisible">
        <img
          style="width: 100%;"
          :src="dialogImageUrl"
          alt=""
...
import { ElUpload, ElImage, ElIcon, ElMessage } from 'element-plus'
...
// 上传成功
const handleSuccess = (_res: any, _file: UploadFile, fileList: UploadFiles) => {
  dataMmit(fileList)
}
// 移除
const remove = (_file: UploadFile, fileList:UploadFiles = []) => {
  dataMmit(fileList)
}
// 上传失败
const handleError = () => {
...
// 事件下发
const dataMmit = (fileList: UploadUserFile[]) => {
